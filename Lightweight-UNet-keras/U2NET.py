...
def u2net(target_size,in_ch=3,out_ch=1):
    ### Additions ###
    model_input = Input(shape=(target_size[0], target_size[1], 3))
    ### End ###
    stage1=RSU7(model_input,in_ch=3,mid_ch=32,out_ch=64)
    ...
    out=Conv2D(out_ch,(1,1),activation='sigmoid',padding='same',name='out')(out)

    # model=Model(inputs=[model_input], outputs=[side1,side2,side3,side4,side5,side6,out])
